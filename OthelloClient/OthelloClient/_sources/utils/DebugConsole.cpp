...
namespace
{
	// Prefixed with DC_ to avoid collision with Windows COLOR_* macros
...
void DebugConsole::Info(const char* fmt, ...)
{
	...
	va_end(args);
	LogInternal(LogType::INFO, buf);
}

void DebugConsole::Warning(const char* fmt, ...)
{
	char buf[BUFFER_SIZE];
	va_list args;
	va_start(args, fmt);
	vsnprintf(buf, sizeof(buf), fmt, args);
	va_end(args);
	LogInternal(LogType::WARNING, buf);
}

void DebugConsole::Error(const char* fmt, ...)
{
	char buf[BUFFER_SIZE];
	va_list args;
	va_start(args, fmt);
	vsnprintf(buf, sizeof(buf), fmt, args);
	va_end(args);
	LogInternal(LogType::ERR, buf);
}

void DebugConsole::Debug(const char* fmt, ...)
{
	char buf[BUFFER_SIZE];
	va_list args;
	va_start(args, fmt);
	vsnprintf(buf, sizeof(buf), fmt, args);
	va_end(args);
	LogInternal(LogType::DEBUG, buf);
}

void DebugConsole::Network(const char* fmt, ...)
{
	char buf[BUFFER_SIZE];
	va_list args;
	va_start(args, fmt);
	vsnprintf(buf, sizeof(buf), fmt, args);
	va_end(args);
	LogInternal(LogType::NETWORK, buf);
}

void DebugConsole::Game(const char* fmt, ...)
{
	char buf[BUFFER_SIZE];
	va_list args;
	va_start(args, fmt);
...
void DebugConsole::SetColor(LogType type) const
{
	...
	const int idx = static_cast<int>(type);
	SetConsoleTextAttribute(m_hConsole, TYPE_COLORS[idx]);
}
